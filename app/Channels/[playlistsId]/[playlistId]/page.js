...
const PlaylistVideosPage = () => {
    ...
    const { playlistsId, playlistId } = router.query || {}; // استخدام قيمة افتراضية في حالة عدم وجود router.query

    console.log('plaulistsId', playlistsId, 'playlistId', playlistId,)
    ...
    useEffect(() => {
        const fetchVideos = async () => {
            if (!playlistId) return; // تأكد من وجود playlistId  قبل استدعاء دالة fetchVideos
        ...
        fetchVideos(); // استدعاء دالة fetchVideos هنا
    }, [playlistId]); // تأكد من أن effect يتغير عندما يتغير playlistId 
    ...
    const filteredVideos = videos
        ...
        .sort((a, b) => {
            ...
            return 0; // ترتيب افتراضي
        });

    const handleVideoClick = (videoUrl) => {
        const videoId = videoUrl.split("v=")[1];
        router.push(`/video/${videoId}`); // استخدم router بدلاً من navigate
    };
